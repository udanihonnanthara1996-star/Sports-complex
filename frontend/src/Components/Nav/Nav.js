import React, { useEffect, useState } from 'react'
import './nav.css';
import { Link, useNavigate } from "react-router-dom";
import { useAuth } from "../Context/AuthContext";
import API from '../../utils/api';

const BADGE_CONFIG = {
  Basic:    { emoji: '🥈', color: '#667eea', label: 'Basic Member' },
  Gold:     { emoji: '🥇', color: '#f6a623', label: 'Gold Member' },
  Platinum: { emoji: '💎', color: '#2c5364', label: 'Platinum Member' },
};

function MembershipBadge() {
  const { auth } = useAuth();
  const navigate = useNavigate();
  const [planName, setPlanName] = useState(null);

  useEffect(() => {
    if (!auth) { setPlanName(null); return; }
    API.get('/api/v1/memberships/my')
      .then(res => {
        if (res.data?.hasActiveMembership) {
          const name =
            res.data.membership?.planId?.name ||
            res.data.membership?.planSnapshot?.name;
          setPlanName(name || null);
        } else {
          setPlanName(null);
        }
      })
      .catch(() => setPlanName(null));
  }, [auth]);

  if (!planName || !BADGE_CONFIG[planName]) return null;

  const { emoji, color, label } = BADGE_CONFIG[planName];

  return (
    <button
      className="nav-membership-badge"
      style={{ '--badge-color': color }}
      onClick={() => navigate('/membership/my')}
      title="View My Membership"
    >
      <span className="badge-emoji">{emoji}</span>
      <span className="badge-label">{label}</span>
    </button>
  );
}

function Nav() {
  const navigate = useNavigate();
  const { logout } = useAuth();

  const handleLogout = () => {
    logout();
    navigate('/');
  };

  return (
    <header className="header">
      <div className="logo">SportSync<span>.LK</span></div>
      <nav className="nav">
        <ul className="nav-list">
          <li><Link to="/home">Home</Link></li>
          <li><Link to="/itemlist">Facility</Link></li>
          <li><Link to="/bookingForm">Booking</Link></li>
          <li><Link to="/customer/events">Event</Link></li>
          <li><Link to="/payment">Payment</Link></li>
          <li><Link to="/membership/plans">Membership</Link></li>
          <li><Link to="/feedback">Feedback</Link></li>
          <li><Link to="/tickets">Tickets</Link></li>
        </ul>
      </nav>
      <div className="auth-buttons" style={{ marginLeft: 'auto' }}>
        {/* Membership badge — appears only when user has active membership */}
        <MembershipBadge />

        {/* Profile avatar button */}
        <button className="profile-btn" onClick={() => navigate('/profile')} title="Profile">
          <ProfileAvatar />
        </button>

        <button onClick={handleLogout} className="logout" title="Logout">
          Logout
        </button>
      </div>
    </header>
  );
}

function ProfileAvatar() {
  const { auth } = useAuth();
  if (!auth) return <span className="avatar-fallback">P</span>;
  const photo = auth?.profile?.photo;
  if (photo) {
    return <img src={photo} alt="Profile" className="nav-avatar" />;
  }
  const name = auth?.name || auth?.email || '';
  const initials = name ? name.split(' ').map(n => n[0]).slice(0, 2).join('').toUpperCase() : 'P';
  return <div className="avatar-fallback">{initials}</div>;
}

export default Nav